...
export default function Button({
  ...
  const hasFill = variant === 'ghost' || variant === 'secondary' || variant === 'primary';

  function handleMouseEnter(e: React.MouseEvent<HTMLButtonElement>) {
    if (variant === 'primary') {
      e.currentTarget.style.color = 'var(--color-text-primary)';
      e.currentTarget.style.transition = 'background 0.5s cubic-bezier(0.16,1,0.3,1), color 0.3s ease';
  ...
  function handleMouseLeave(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    if (variant === 'ghost') {
      e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--color-sand-950) 40%, transparent)';
  ...
  const borderStyle = variant === 'ghost'
    ? { borderColor: 'color-mix(in srgb, var(--color-sand-950) 40%, transparent)', outline: 'none' }
    : { outline: 'none' };

  const classes = [
    `inline-flex ${size === 'xl' ? 'h-14' : 'h-11'} justify-center items-center gap-3`,
  ...
  return (
    ...
    >
      ...
      <span className="relative z-10 inline-flex items-center gap-3 pointer-events-none">
        {iconName && <Icon name={iconName} size="md" color={variant === 'primary' ? 'light' : 'dark'} />}
        {!iconOnly && label}
