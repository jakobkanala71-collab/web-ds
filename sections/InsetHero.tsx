    ...
    <section className="bg-background">
      ...
      <div
        ...
        style={{
          ...
          paddingBottom: isMobile ? `${BADGE_H - OVERLAP}px` : `${bPad}px`
        }}
      >
        <div
          ...
          style={{
            ...
            clipPath: ready
              ? isMobile
                ? `inset(0 0 0% 0 round 0 0 0 ${radius}px)`
                : `inset(0 0 0% 0 round 0 0 ${radius}px ${radius}px)`
              : isMobile
              ? `inset(0 0 100% 0 round 0 0 0 ${radius}px)`
              : `inset(0 0 100% 0 round 0 0 ${radius}px ${radius}px)`,
            transition: ready
              ? "clip-path 1s cubic-bezier(0.7,0,0.3,1) 0ms"
              : "none",
      ...
      {/* ── Text + CTAs ─────────────────────────────────────────────── */}
      <div className="relative z-[5] max-w-[var(--breakpoint-xl)] mx-auto">
        <div className="px-8 md:px-6 pt-1 md:pt-8 pb-16">
          <div className="flex flex-col gap-4 max-w-2xl items-start text-left">
            <div
              style={{
                opacity: ready ? 1 : 0,
                transform: ready ? "translateX(0)" : "translateX(-2rem)",
                transition: t(1020)
              }}
            >
              <ResponsiveHeading mobile="h4" desktop="h3">
                {heading}
              </ResponsiveHeading>
            </div>
            ...
            >
              <Text variant="p-medium" color="body">
                {body}
              </Text>
            </div>
            ...
            <div
              style={{
                opacity: ready ? 1 : 0,
                transform: ready ? "translateY(0)" : "translateY(1rem)",
                transition: t(1300),
                marginTop: "1.5rem",
            ...
            >
              <div
                className={`flex gap-3 ${
                  isMobile ? "flex-col items-start" : "flex-row flex-wrap"
                }`}
              >
                {!isMobile && (
                  <Button
                ...
                {secondaryCtaLabel && (
                  <Button
                    label={
                      isMobile && mobileSecondaryCtaLabel
                        ? mobileSecondaryCtaLabel
                        : secondaryCtaLabel
                    }
                    variant={isMobile ? "primary" : "ghost"}
                    size={isMobile ? "xl" : "default"}
                    iconElement={secondaryCtaIcon}
                    rounded
